...
| `WindWheel` | 旋转风轮 | 角色身边环绕刀刃，持续接触伤害 |
| `LaserGun` | 激光枪 | 朝最近敌人发射激光束（线段判定） |
| `SwordEnergy` | 剑气 | 发射高穿透（999）剑气弹 |
| `Turret` | 炮台 | 在身边放置自动射击的炮台实体（有持续时间） |
| `LandMine` | 地雷 | 在脚下布雷，敌进入爆炸半径触发 AoE |
...
- **鼠标**：移动即设定攻击方向（带 0.3s 手动瞄准优先窗口，超时则自动朝最近敌人/Mini-boss）；点击可在各菜单直接点选。
...
- HUD：等级、生命、击杀、武器槽（含弹药条/换弹旋转环）、底部经验条；暂停面板展示完整属性面板。
...
2. `pollGamepad` 采集手柄。
3. 若 `Playing`：
   - `getMovementDirection` 取移动向量；`updateMouseDirection` 取攻击方向（含 0.3s 手动瞄准优先 + 超时自动索敌）。
